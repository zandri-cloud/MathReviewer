...
export function QuestionCardGlassUI({ question, index, total, answer, onAnswerChange }: { question: ExamProblem, index: number, total: number, answer: string, onAnswerChange: (ans: string) => void }) {
  return (
    ...
    >
      ...
      <div className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-serif text-on-surface leading-relaxed min-h-[100px] sm:min-h-[120px] flex flex-col items-center justify-center overflow-x-auto py-4 gap-4">
        {question && <MathText block math={question.statement} className="text-center" />}
        {question && <ProblemDiagram statement={question.statement} />}
      </div>
